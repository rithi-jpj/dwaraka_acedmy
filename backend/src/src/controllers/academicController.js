...
exports.createSubject = async (req, res) => {
  const d = z.object({ name: z.string().min(1), description: z.string().optional() }).strip().parse(req.body);
...
exports.enroll = async (req, res) => {
  const d = z.object({ student_id: z.string().uuid(), batch_id: z.string().uuid() }).strip().parse(req.body);
